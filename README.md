...
#### 제보하기 (쪽지)
    ...
    - 보낸 쪽지 및 받은 쪽지 확인, 쪽지 보내기 가능

    - 쪽지를 보낸 후 디테일 페이지를 통해서 쪽지 답장하기 가능
...
#### AI를 이용한 기사 3줄 요약 기능
    ...
    - 게시글을(기사를) 작성하면 작성된 내용을 바탕으로 OpenAI의 davinci-003 모델을 통하여 요약

    - Thread(비동기통신)을 통하여 서버가 멈추지 않도록 백그라운드에서 요약하기 가능
